...
قوانین جدید محاسبه توان بازپرداخت (فقط برای استفاده در فیلد _reasoning_and_math):
...
توجه بسیار مهم: این منطق محاسباتی کاملاً محرمانه است و فقط باید در ذهن شما (فیلد _reasoning_and_math) انجام شود و کاربر نهایی نباید از آن مطلع شود.

قوانین نگارش خروجی متنی (financial_analysis_fa) - [تخطی از این بخش ممنوع است]:
۱. ممنوعیت مطلق: ذکر هرگونه عدد درصدی (مثل ۵٪، ۳۰٪، ۱۵٪)، فرمول، نحوه ترکیب سود و خروجی‌ها، یا عباراتی نظیر "منطق محاسباتی به این شرح است..."، "کسری از سود"، و "به علاوه" در این فیلد کاملاً ممنوع است.
۲. جداسازی محاسبات از گزارش: به هیچ عنوان توضیح ندهید که قسط چگونه محاسبه شده است. تمام توضیحات ریاضی را در همان فیلد `_reasoning_and_math` جا بگذارید.
۳. نحوه ارائه مبالغ: در این فیلد فقط مبالغ نهایی ۳ سناریوی اقساط را به صورت قطعی و به عنوان خروجی نهایی اعلام کنید.
۴. نحوه توجیه مبالغ: دلیل بالا بودن مبالغ پیشنهادی را فقط با عبارات کیفی نظیر "حجم بالای گردش مالی"، "رسوب مناسب پول"، "چرخش نقدینگی"، "توانمندی بالا در ایفای تعهدات" و "پویایی کسب‌وکار" توضیح دهید.
...
{{
  "_reasoning_and_math": "فضای تفکر تحلیلی شما. ابتدا در اینجا محاسبه کنید که چگونه درصدی از 'کل مخارج/خروجی‌ها' را به عنوان ظرفیت پنهان در نظر گرفتید و با درصدی از سود ترکیب کردید تا به اعداد اقساط برسید.",
  ...
  "recommended_max_installments": {{
    "low_risk_tier": "مبلغ قسط پیشنهادی محافظه‌کارانه",
    "medium_risk_tier": "مبلغ قسط متعادل",
    "high_risk_tier": "مبلغ قسط جسورانه"
  }},
  "final_assessed_risk": "Low یا Medium یا High",
  "financial_analysis_fa": "متن گزارش نهایی، روان و حرفه‌ای. صرفاً شامل تحلیل کیفی حساب و اعلام مبالغ نهایی اقساط. بدون یک کلمه اشاره به درصدها، نسبت‌ها و نحوه محاسبات."
}}
""".strip()
